...
(function() {
	...
	var Rectangle = function(anX, aY, aWidth, aHeight) {
		this.origin = new Point(anX, aY);
	...
	Rectangle.prototype = {
		...
		intersects: function(aRectangle) {
			...
			if (rCorner.x <= origin.x) return false;
			if (rCorner.y <= origin.y) return false;
			if (rOrigin.x >= corner.x) return false;
			if (rOrigin.y >= corner.y) return false;
			return true;
		},
		toString: function() {
	...
	geometry.rectangle = function(anX, aY, aWidth, aHeight) { return new Rectangle(anX, aY, aWidth, aHeight); };
	geometry.point = function(anX, aY) { return new Point(anX, aY); };
})()